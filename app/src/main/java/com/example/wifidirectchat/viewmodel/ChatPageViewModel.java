...
public class ChatPageViewModel extends AndroidViewModel {
    ...
    //It is set when we find a partner device (the observer shows the chat page)
    private MutableLiveData<Boolean> chatIsReady;

    //It is automatically added from the database when a new message is added to the database with the corresponding name
    private LiveData<List<MessageEntity>> messageList;

    //Occurs when a new proprietary device appears
    private MutableLiveData<List<WifiP2pDevice>> peerList;

    //Occurs when one of the devices closes the chat (the observer throws the corresponding message)
    ...
    public ChatPageViewModel(@NonNull final Application application) {
        ...
        WifiP2pManager.ConnectionInfoListener connectionInfoListener = new WifiP2pManager.ConnectionInfoListener() {
            ...
            public void onConnectionInfoAvailable(WifiP2pInfo info) {
                ...
                Toast.makeText(application, "Connecting to the device", Toast.LENGTH_SHORT).show();
            }
        };
